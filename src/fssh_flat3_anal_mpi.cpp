...
#include "2d_flat3_potential.hpp"
...
vector< complex<double> > dx_dcx(4), dy_dcx(4), dx_dcy, dy_dcy(4);
...
int hopper(state_t& state) {
    ...
    if (randomer::rand() < g) {
        ...
        vector<double> n = get_rescaling_direction(rescaling_alg, r, v, c, from, to,
                                                        dcx, dcy, dx_dcx, dy_dcx, dx_dcy, dy_dcy,
                                                        Fx, Fy, eva);
        // rescale momentum
        if (norm(n) > 1e-40) {
        ...
        for (int itraj(0); itraj < my_Ntraj; ++itraj) {
            if (check_end(state[itraj]) == false) {
                ...
                cal_info_anal(
                        ...
                        Fx, Fy, dcx, dcy, dx_dcx, dy_dcx, dx_dcy, dy_dcy, eva, lastevt
                        );
                // hopper
                if (enable_hop) {
