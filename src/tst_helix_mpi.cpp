...
double cal_crossing_E0(const double x) {
    ...
    // ASSUMING crossig line is y = x
    const double y = x;
    return 0.5 * (param_A * pow(x, 2) + pow(y - param_B, 2) + param_D) - param_C;
}

double P_initx(const double x) {
    // initial position distribution probability
    return exp(-1.0 / kT * cal_crossing_E0(x));
}

void init_state(vector<state_t>& state) {
    // state = x, y, vx, vy, c0, c1, s
    size_t Ntraj = state.size();

    // init on the tranistion state
    // sample initial position
    vecto<double> xarr = randomer::MHsample(P_initx, Ntraj, 10000, 100, randomer::rand(), 0.2);
    for (int itraj(0); itraj < Ntraj; ++itraj) {
        state_t& st = state[itraj];
...
void sys(const state_t& state, state_t& state_dot, const double /* t */) {
    ...
    //debug
    axmax = max(abs(state_dot[2]), axmax);
    aymax = max(abs(state_dot[3]), aymax);
}
...
void fssh() {
    ...
    if (MPIer::master) {
        ...
        ioer::info("# debug:", " axmax = ", axmax, " aymax = ", aymax);
    }
    MPIer::barrier();
}
