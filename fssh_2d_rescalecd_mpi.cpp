...
int hopper(state_t& state) {
    ...
    if (randomer::rand() < g) {
        // adjust momentum
        vector<double> n(2);
        n[0] = (c[0] * conj(c[0]) * dcx[0+0*2] + c[1] * conj(c[1]) * dcx[1+1*2] + c[0] * conj(c[1]) * dcx[1+0*2] + c[1] * conj(c[0]) * dcx[0+1*2]).real();
        n[1] = (c[0] * conj(c[0]) * dcy[0+0*2] + c[1] * conj(c[1]) * dcy[1+1*2] + c[0] * conj(c[1]) * dcy[1+0*2] + c[1] * conj(c[0]) * dcy[0+1*2]).real();
