...
void fssh() {
    ...
    for (int istep(0); istep < Nstep; ++istep) {
        ...
        if (istep % output_step == 0) {
            ...
            n0trans_arr[irec] = n0refl_arr[irec] = n1trans_arr[irec] = n1refl_arr[irec] = 0.0;
            px0trans_arr[irec] = px0refl_arr[irec] = px1trans_arr[irec] = px1refl_arr[irec] = 0.0;
            py0trans_arr[irec] = py0refl_arr[irec] = py1trans_arr[irec] = py1refl_arr[irec] = 0.0;
            ...
            for (int itraj = 0; itraj < my_Ntraj; ++itraj) {
                ...
                if (vx >= 0.0) {
                    n0trans_arr[irec] += n0d;
                    n1trans_arr[irec] += n1d;
                    px0trans_arr[irec] += mass * vx * n0d;
                    px1trans_arr[irec] += mass * vx * n1d;
                    py0trans_arr[irec] += mass * vy * n0d;
                    py1trans_arr[irec] += mass * vy * n1d;
                }
                else {
                    n0refl_arr[irec] += n0d;
                    n1refl_arr[irec] += n1d;
                    px0refl_arr[irec] += mass * vx * n0d;
                    px1refl_arr[irec] += mass * vx * n1d;
                    py0refl_arr[irec] += mass * vy * n0d;
                    py1refl_arr[irec] += mass * vy * n1d;
                }
            }
