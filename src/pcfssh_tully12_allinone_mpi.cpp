...
const set<string> pc_rescaling_alg_set { "none", "parallel", "x", "red", "deltaF" };
string pc_frustration_alg = "neil";
const set<string> pc_frustration_alg_set { "neil", "joe" };
...
void sys(const state_t& state, state_t& state_dot, const double /* t */) {
    ...
    if (pc_rescaling_alg == "none") {
        ...
        else if (pc_rescaling_alg == "deltaF") {
            // TODO: WHAT IF DF = 0?
            //n[0] = (Fx[1-s+(1-s)*2] - Fx[s+s*2]).real();
            //n[1] = (Fy[1-s+(1-s)*2] - Fy[s+s*2]).real();
        }
