#ifndef _2D_TULLY3_POTENTIAL_HPP
#define _2D_TULLY3_POTENTIAL_HPP
...
namespace {
    ...
    double param_A = 6.0e-4;
    double param_B = 0.10;
    double param_C = 0.90;
    double param_W = 0.0;

    void output_potential_param() {
        ioer::info("# 2D Tully3 potential parameters: ", 
                    " A = ", param_A,
                    " B = ", param_B,
                    " C = ", param_C,
                    " W = ", param_W);
    ...
    vector< complex<double> > cal_H(const vector<double>& r) {
        ...
        H[1+1*2] = -param_A;
        if (x < 0.0) {
            H[0+1*2] = param_B * exp(param_C * x) * eip;
        }
        else {
            H[0+1*2] = param_B * (2.0 - exp(-param_C * x)) * eip;
        }
    ...
    vector< complex<double> > cal_nablaHx(const vector<double>& r) {
        ...
        if (x < 0.0) {
            nablaHx[0+1*2] = param_B * param_C * exp(param_C * x) * eip;
        }
        else {
            nablaHx[0+1*2] = param_B * param_C * exp(-param_C * x) * eip;
        }
    ...
    vector< complex<double> > cal_nablaHy(const vector<double>& r) {
        ...
        if (x < 0.0) {
            nablaHy[0+1*2] = param_B * exp(param_C * x) * eip * matrixop::IMAGIZ * der_phi;
        }
        else {
            nablaHy[0+1*2] = param_B * (2.0 - exp(-param_C * x)) * eip * matrixop::IMAGIZ * der_phi;
        }
