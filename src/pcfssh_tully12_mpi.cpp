...
void sys(const state_t& state, state_t& state_dot, const double /* t */) {
    ...
    */
    ...
    if (s == 0) {
        ...
        const double v0sq = v0[0] * v0[0] + v0[1] * v0[1];
        const double v1sq = v0sq + 2.0 / mass * (eva[0] - eva[1]);
        const vector<double> v1 = v0 * (v1sq > 0.0 ? sqrt(v1sq / v0sq) : 0.0);
        ...
        const double v1sq = v1[0] * v1[0] + v1[1] * v1[1];
        const double v0sq = v1sq + 2.0 / mass * (eva[1] - eva[0]);
        const vector<double> v0 = v1 * (v0sq > 0.0 ? sqrt(v0sq / v1sq) : 0.0);
...
int hopper(state_t& state) {
    ...
    if (randomer::rand() < g) {
        ...
        if (rescaling_alg == "b1") {
            ...
            complex<double> coef = 
                  Fx[to+from*2] * (Fx[to+to*2] - Fx[from+from*2]) / dE / dE - Hxx[to+from*2] / dE 
                + Fy[to+from*2] * (Fy[to+to*2] - Fy[from+from*2]) / dE / dE - Hyy[to+from*2] / dE 
                ;
            n.resize(2);
            n[0] = (coef * dcx[from+to*2]).real();
