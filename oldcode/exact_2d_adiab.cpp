...
void exact() {
    ...
    vector< complex<double> > evt00(M*M), evt01(M*M), evt10(M*M), evt11(M*M);
    vector< double > eva00(M*M), eva11(M*M);
    vector< double > pad00(M*M), pad11(M*M);
    for (int j = 0; j < M; ++j) {
        double x = xarr[j];
        for (int k = 0; k < M; ++k) {
            double y = yarr[k];
            ...
            /*
            evt00[k+j*M] = evt[0+0*2];
            evt10[k+j*M] = evt[1+0*2];
            evt01[k+j*M] = evt[0+1*2];
            evt11[k+j*M] = evt[1+1*2];
            complex<double> phase = 0.0;
            if (j == 0 and k == 0) {
            }
            else if (j == 0) {
                phase = conj(evt00[(k-1)+j*M]) * evt00[k+j*M] + conj(evt10[(k-1)+j*M]) * evt10[k+j*M];
                phase /= abs(phase);
                evt00[k+j*M] /= phase;
                evt10[k+j*M] /= phase;

                phase = conj(evt01[(k-1)+j*M]) * evt01[k+j*M] + conj(evt11[(k-1)+j*M]) * evt11[k+j*M];
                phase /= abs(phase);
                evt01[k+j*M] /= phase;
                evt11[k+j*M] /= phase;
            }
            else {
                phase = conj(evt00[k+(j-1)*M]) * evt00[k+(j-1)*M] + conj(evt10[k+(j-1)*M]) * evt10[k+(j-1)*M];
                phase /= abs(phase);
                evt00[k+(j-1)*M] /= phase;
                evt10[k+(j-1)*M] /= phase;

                phase = conj(evt01[k+(j-1)*M]) * evt01[k+(j-1)*M] + conj(evt11[k+(j-1)*M]) * evt11[k+(j-1)*M];
                phase /= abs(phase);
                evt01[k+(j-1)*M] /= phase;
                evt11[k+(j-1)*M] /= phase;
            }
            */
            ...
            evt00[k+j*M] = cos(0.5 * theta) * eip;
            evt01[k+j*M] = sin(0.5 * theta) * eip;
            evt10[k+j*M] = -sin(0.5 * theta);
            evt11[k+j*M] = cos(0.5 * theta);

            pad00[k+j*M] = pow(cos(0.5 * theta), 2) * der_phi;
            pad11[k+j*M] = pow(sin(0.5 * theta), 2) * der_phi;
            /*
            pad00[k+j*M] = 0.0;
            pad11[k+j*M] = 0.0;
            */
    ...
    vector< complex<double> > psi_kad0(M * M, 0.0), psi_kad1(M * M, 0.0);
    ...
    for (int istep = 0; istep < Nstep; ++istep) {
        ...
        // analysis & output ON ADIAB
        if (istep % output_step == 0) {
            double nm;
            // get psi_k, psiad, and psi_kad
            psi_k0 = myfftshift(misc::fftn(psi0, dim));
            psi_k1 = myfftshift(misc::fftn(psi1, dim));

            nm = norm(psi_k0 | psi_k1);
            psi_k0 /= nm;
            psi_k1 /= nm;

            psiad0 = conj(evt00) * psi0 + conj(evt10) * psi1;
            psiad1 = conj(evt01) * psi0 + conj(evt11) * psi1;

            psi_kad0 = myfftshift(misc::fftn(psiad0, dim));
            psi_kad1 = myfftshift(misc::fftn(psiad1, dim));

            nm = norm(psi_kad0 | psi_kad1);
            psi_kad0 /= nm;
            psi_kad1 /= nm;
            KE0 = KE1 = PE = 0.0;
            ...
            for (int k = 0; k < M; ++k) {
                for (int j = 0; j < M; ++j) {
                    if (kxarr[j] >= 0.0) {
                        n0trans += pow(abs(psi_kad0[k+j*M]), 2);
                        n1trans += pow(abs(psi_kad1[k+j*M]), 2);
                        py0trans += pow(abs(psi_kad0[k+j*M]), 2) * kyarr[k] + pow(abs(psiad0[k+j*M]), 2) * pad00[k+j*M];
                        py1trans += pow(abs(psi_kad1[k+j*M]), 2) * kyarr[k] + pow(abs(psiad1[k+j*M]), 2) * pad11[k+j*M];
                        px0trans += pow(abs(psi_kad0[k+j*M]), 2) * kxarr[j];
                        px1trans += pow(abs(psi_kad1[k+j*M]), 2) * kxarr[j];
                    }
                    else {
                        n0refl += pow(abs(psi_kad0[k+j*M]), 2);
                        n1refl += pow(abs(psi_kad1[k+j*M]), 2);
                        py0trans += pow(abs(psi_kad0[k+j*M]), 2) * kyarr[k] + pow(abs(psiad0[k+j*M]), 2) * pad00[k+j*M];
                        py1trans += pow(abs(psi_kad1[k+j*M]), 2) * kyarr[k] + pow(abs(psiad1[k+j*M]), 2) * pad11[k+j*M];
                        px0refl += pow(abs(psi_kad0[k+j*M]), 2) * kxarr[j];
                        px1refl += pow(abs(psi_kad1[k+j*M]), 2) * kxarr[j];
                    }
            ...
            PE = real(sum(conj(psiad0) * eva00 * psiad0 + conj(psiad1) * eva11 * psiad1));
            // output
            if (istep == 0) {
                ioer::info("# EXACT 2D ");
